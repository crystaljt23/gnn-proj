...
# problem setting from https://github.com/pyg-team/pytorch_geometric/blob/master/examples/link_pred.py
# implementation of https://arxiv.org/abs/1904.03751
...
class ResBlock(torch.nn.Module):
    def __init__(self, num_channels):
        super().__init__()
        self.conv1 = GCNConv(num_channels, num_channels)
        self.conv2 = GCNConv(num_channels, num_channels)
        self.p = Linear(num_channels, num_channels)
...
class Net(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_blocks, mlp_depth):
        super().__init__()
        ...
        self.mlp = torch.nn.ModuleList(
            [Linear(hidden_channels, hidden_channels) for _ in range(mlp_depth)]
        )
...
model = Net(dataset.num_features, 128, 64, 6, 1).to(device)
